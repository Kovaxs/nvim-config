...
keymap.set("n", "<leader>cl",
    function() vim.cmd("setlocal spelllang=" .. (vim.bo.spelllang == "es" and "en" or "es")) end,
    { desc = "Toggle spell check language between Spanish and English" })
...
keymap.set('n', '<leader>fg', require('telescope.builtin').git_files,
    { desc = "Opens a Telescope prompt to search and navigate your Git repository files." })                                                                                                                                                                                                           -- Opens a Telescope prompt to search and navigate your Git repository files.
keymap.set('n', '<leader>fh', require('telescope.builtin').help_tags,
    { desc = "Opens a Telescope prompt to search and access Neovim help tags." })                                                                                                                                                                                                                      -- Opens a Telescope prompt to search and access Neovim help tags.
keymap.set('n', '<leader>ff', function() require('telescope.builtin').find_files({ hidden = true }) end,
    { desc = "Opens a Telescope prompt to search and open files on your system." })                                                                                                                                                                                                                    -- Opens a Telescope prompt to search and open files on your system.
keymap.set('n', '<leader>fa', require('telescope.builtin').live_grep,
    { desc = "Opens a Telescope prompt to search real-time across your project files using a user-provided pattern." })                                                                                                                                                                                -- Opens a Telescope prompt to search real-time across your project files using a user-provided pattern.
keymap.set('n', '<leader>fb', require('telescope.builtin').buffers,
    { desc = "Opens a Telescope prompt to switch and manage open buffers in Neovim." })                                                                                                                                                                                                                -- Opens a Telescope prompt to switch and manage open buffers in Neovim.
keymap.set('n', '<leader>fs', require('telescope.builtin').current_buffer_fuzzy_find,
    { desc = "Opens a Telescope prompt to search within the currently open buffer." })                                                                                                                                                                                                                 -- Opens a Telescope prompt to search within the currently open buffer.
keymap.set('n', '<leader>fo', require('telescope.builtin').lsp_document_symbols,
    { desc =
    "Opens a Telescope prompt to search and navigate symbols within the current document using Language Server Protocol (LSP) integration." })                                                                                                                                                         -- Opens a Telescope prompt to search and navigate symbols within the current document using Language Server Protocol (LSP) integration.
keymap.set('n', '<leader>fi', require('telescope.builtin').lsp_incoming_calls,
    { desc = "Opens a Telescope prompt to search for references to the currently selected symbol using LSP integration." })                                                                                                                                                                            -- Opens a Telescope prompt to search for references to the currently selected symbol using LSP integration.
keymap.set('n', '<leader>fm', function() require('telescope.builtin').treesitter({ default_text = ":method:" }) end,
    { desc =
    "This custom function utilizes Telescope's treesitter provider and sets the default text to 'method:', likely used for finding and selecting methods within your code." })                                                                                                                         -- This custom function utilizes Telescope's treesitter provider and sets the default text to ":method:", likely used for finding and selecting methods within your code.
...
keymap.set("n", "<leader>hg1",
    function()
        require("harpoon.tmux").sendCommand("2", 1); require("harpoon.tmux").gotoTerminal("2")
...
-- keymap.set('n', '<leader>li', '<cmd>lua vim.lsp.buf.implementation()<CR>', {desc = "LSP: Go to Implementation"}) -- Don't work
...
-- keymap.set('n', '<leader>lt', '<cmd>lua vim.lsp.buf.type_definition()<CR>', {desc = "LSP: Type Definition"}) -- Don't work in python
...
keymap.set("n", '<leader>de', function() require('telescope.builtin').diagnostics({ default_text = ":E:" }) end,
    { desc = "Shows diagnostics (errors) using Telescope with a default filter for ':E:'" })
...
keymap.set("n", '<leader>di', function() require "dap.ui.widgets".hover() end,
    { desc = "Shows the value of the variable under the cursor" })
...
keymap.set({ "n", "v" }, "<leader>at", "<cmd>ChatGPTRun translate<CR>",
...
keymap.set({ "n", "v" }, "<leader>aa", "<cmd>ChatGPTRun add_tests<CR>",
